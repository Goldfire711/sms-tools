...
public:
  ...
  static void hook();
  static void unHook();
  static bool readFromRAM(const u32 offset, u8* buffer, const size_t size, const bool withBSwap);
  static bool writeToRAM(const u32 offset, const u8* buffer, const size_t size,
  ...
  static std::string getFormattedValueFromCache(const u32 ramIndex, Common::MemType memType,
                                                ...
                                                bool memIsUnsigned);
  static void copyRawMemoryFromCache(u8* dest, const u32 consoleAddress, const size_t byteCount);
  static bool isValidConsoleAddress(const u32 address);

private:
  static IDolphinProcess* m_instance;
  static DolphinStatus m_status;
  static u8* m_updatedRAMCache;
};
} // namespace DolphinComm
