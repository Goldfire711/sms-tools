...
bool WindowsDolphinProcess::readFromRAM(const u32 offset, u8* buffer, const size_t size,
...
bool WindowsDolphinProcess::writeToRAM(const u32 offset, const u8* buffer, const size_t size,
                                       const bool withBSwap) {
  u64 RAMAddress = m_emuRAMAddressStart + offset;
  SIZE_T nread = 0;
  auto bufferCopy = new char[size];
